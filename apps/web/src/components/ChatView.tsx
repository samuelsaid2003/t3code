...
type ChatViewWorkspaceProps = {
  /** Multi-thread desktop panes keep the full action header and auxiliary
...
function ChatViewContent(props: ChatViewProps) {
  ...
  const citeAssistantText = useCallback(
    (citation: AssistantCitation, sourceAnchor: AssistantCitationSourceAnchor) => {
      const inserted = localComposerRef.current?.citeAssistantText(citation, sourceAnchor) ?? false;
      if (!inserted) {
        toastManager.add({
          type: "warning",
  ...
  const rightPanelPresence = usePanelPresence(
    visibleRightPanelOpen && activeThreadRef !== null,
  ...
  const mountComposerContextStrip = shouldShowComposerContextStrip({
    hasActiveProject: runContextPlacement === "composer" && activeProject !== null,
    isGitRepo,
    showEnvironmentIndicator: showComposerEnvironmentIndicator,
    ...
    (surface: RightPanelSurface) => {
      ...
      const finishClose = () => {
        void deleteSideChatSurfaces(surfaces).then((deleted) => {
          if (deleted) finishRightPanelSurfaceClose(surfaces);
        });
      };
  ...
  const closeAllRightPanelSurfaces = useCallback(() => {
    ...
    const finishClose = () => {
      void deleteSideChatSurfaces(rightPanelState.surfaces).then((deleted) => {
        if (deleted) finishRightPanelSurfaceClose(rightPanelState.surfaces);
      });
    };
  ...
  const handlePullRequestTabStatusChange = useCallback(
    (status: { repository: string; number: number; state: "open" | "closed" | "merged" }) => {
      if (
  ...
  const panelToggleControls = (
    <PanelLayoutControls
      ...
      liveAgentCount={
        visibleRightPanelOpen && activeRightPanelSurface?.kind === "agents"
          ? 0
          : agentPanelModel.liveCount
      }
  ...
  const panelLayoutControls = (
    ...
    >
      {!shouldUseRightPanelSheet ? (
        <span
          aria-hidden={!visibleRightPanelOpen}
          className={cn(
            "flex shrink-0",
            panelAnimationsActive &&
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "side-chat" ? (
      activeSideThreadShell?.session?.status === "error" ? (
        <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 px-8 text-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              size="sm"
              variant="ghost"
              onClick={() => closeRightPanelSurface(renderedRightPanelSurface)}
            >
              Close
            </Button>
          </div>
        ...
        <ChatViewContent
          key={renderedRightPanelSurface.resourceId}
          environmentId={activeThreadRef.environmentId}
      ...
      <PullRequestDetailPanel
        ...
        {...(linkedThreadPullRequest === null
          ...
          : {})}
      />
    ) : renderedRightPanelSurface?.kind === "agent-profile" ? (
    ...
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      >
        ...
        {!sidePanelPresentation ? (
          ...
          >
            {!visibleRightPanelOpen || !rightPanelControlsInPanel ? panelLayoutControls : null}
            <ChatHeader
                  ...
                  >
                    <ComposerSurface.Shell contextStrip={showComposerContextStrip}>
                      ...
                      <div className="min-h-0">
                        ...
                        >
                          {mountComposerContextStrip && runContextProps ? (
                            <div className="pointer-events-auto">
                              <BranchToolbar
                                {...runContextProps}
                                showGitControls={runContextPlacement === "composer" && isGitRepo}
        ...
        {mountedTerminalThreadRefs.map(({ key: mountedThreadKey, threadRef: mountedThreadRef }) => (
          <PersistentThreadTerminalDrawer
            ...
            active={workspaceFocused && mountedThreadKey === activeThreadKey}
            launchContext={
              workspaceFocused && mountedThreadKey === activeThreadKey
                ? (activeTerminalLaunchContext ?? null)
                : null
            }
            focusRequestId={
              workspaceFocused && mountedThreadKey === activeThreadKey ? terminalFocusRequestId : 0
            }
            splitShortcutLabel={splitTerminalShortcutLabel ?? undefined}
            splitVerticalShortcutLabel={splitTerminalVerticalShortcutLabel ?? undefined}
            newShortcutLabel={newTerminalShortcutLabel ?? undefined}
...
export default memo(ChatViewContent);
