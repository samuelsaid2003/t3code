...
export default function FilePreviewPanel({
  ...
  const file = useProjectFileQuery(
    ...
    cwd,
    relativePath,
    attachment === undefined && !(isMedia && !isHistoricalRef) && !isPdf,
    selectedRef,
  );
  ...
  const renderBrowserFile =
    !isHistoricalRef && (isPdf || (isHtml && renderBrowserFilePreferred && revealHandled));
  const canToggleRendered = !isHistoricalRef && attachment === undefined && (isMarkdown || isHtml);
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        >
          {relativePath && attachment ? (
          ...
          ) : relativePath && !isHistoricalRef && isVideo && absolutePath ? (
            <WorkspaceVideoPreview
              ...
              workspaceMutationId={workspaceMutationId}
            />
          ) : relativePath && !isHistoricalRef && isImage && absolutePath ? (
