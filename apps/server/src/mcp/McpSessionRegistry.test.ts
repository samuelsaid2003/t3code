...
it.effect("preserves only the capabilities requested for one provider session", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry(() => 1_000);
    const issued = yield* registry.issue({
      threadId: ThreadId.make("thread-capabilities"),
      providerInstanceId: ProviderInstanceId.make("codex"),
      capabilities: new Set(["agent-routines", "task-management"]),
    });
    const token = issued.config.authorizationHeader.replace(/^Bearer\s+/, "");
    const resolved = yield* registry.resolve(token);

    expect(resolved?.capabilities.has("preview")).toBe(false);
    expect(resolved?.capabilities.has("agent-routines")).toBe(true);
    expect(resolved?.capabilities.has("task-management")).toBe(true);
  }),
);
