...
export interface CodexSessionRuntimeOptions {
  ...
  readonly forkFromResumeCursor?: CodexResumeCursor;
...
export const openCodexThread = (input: {
  ...
  if (input.forkFromThreadId !== undefined) {
    return input.client
      ...
      .pipe(
        Effect.tap((opened) =>
          input.client.request("thread/inject_items", {
  ...
  Effect.gen(function* () {
    ...
    const start = Effect.fn("CodexSessionRuntime.start")(function* () {
      ...
      const opened = yield* openCodexThread({
        ...
        forkFromThreadId: readResumeCursorThreadId(options.forkFromResumeCursor),
      });
